            ...
            <div className="flex flex-col gap-8 py-2 relative">
              <div>
                ...
                <p className="text-xs text-gray-400 mt-2">
                  I help businesses grow their online presence through SEO, social media
                  marketing, and targeted ad campaigns. My focus is on creating data-driven
                  strategies that boost visibility, attract the right audience, and convert
              ...
              <ul className="flex flex-col gap-4">
                {navLinksdata.map((item) => (
                  <li key={item._id} className="text-base font-normal text-gray-400 tracking-wide cursor-pointer hover:text-designColor duration-300">
                    <Link onClick={() => setShowMenu(false)} activeClass="active" to={item.link} spy={true} smooth={true} offset={-70} duration={500}>{item.title}</Link>
                  </li>
                ))}
              </ul>
              <div className="flex flex-col gap-4">
                ...
                <div className="flex gap-4">
                  <span onClick={() => { window.open(window.links.whatsapp, "_blank") }} className="bannerIcon"><FaWhatsapp /></span>
                  <span onClick={() => { window.open(window.links.facebook, "_blank") }} className="bannerIcon"><FaFacebookF /></span>
                  <span onClick={() => { window.open(window.links.linkedIn, "_blank") }} className="bannerIcon"><FaLinkedinIn /></span>
                </div>
              </div>
